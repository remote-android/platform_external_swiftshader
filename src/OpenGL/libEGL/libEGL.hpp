...
private:
	LibEGLexports *loadExports()
	{
		if(!libEGL)
		{
				...
				#if defined(__LP64__)
					const char *libEGL_lib[] = {"/vendor/lib64/" ANDROID_RELATIVE_PATH "/libEGL_swiftshader.so", "/system/lib64/" ANDROID_RELATIVE_PATH "/libEGL_swiftshader.so"};
				#else
					const char *libEGL_lib[] = {"/vendor/lib/" ANDROID_RELATIVE_PATH "/libEGL_swiftshader.so", "/system/lib/" ANDROID_RELATIVE_PATH "/libEGL_swiftshader.so"};
				#endif
